export default function Home() {
  ...
  <div className="flex grid grid-cols-1 h-screen w-screen bg-gray-200" id="">
             ...
             <h3 className="text-4xl text-center font-extrabold uppercase bg-gradient-to-br from-green-400 via-sky-600 to-blue-300 dark:bg-gradient-to-r from-blue-400 via-green-200 to-purple-900 text-transparent bg-clip-text p-2 md:text-4xl">Basic website:</h3>
  
          <ul className="flex grid grid-cols-1 p-10 z-0 list-disc bg-gradient-to-tl from-blue-600 via-sky-600 to-blue-300 md:h-full">
          ...
          <img src="calendario.png" className="w-10 p-1 md:w-10 animate-[pulse_2s_ease-in-out_infinite] " alt="calendarIcon" />
        ...
        {/**   <a href="landing" target='_blank'> <button className="absolute -bottom-6 md:-bottom-10 left-1/2 transform -translate-x-1/2 text-xl bg-gray-700 px-4 py-2 rounded-full text-black z-10 border-[1px] border-gray-400 md:text-2xl"><span className="bg-gradient-to-br from-green-400 via-sky-600 to-blue-300 text-transparent bg-clip-text font-bold uppercase">E.g of Basic website</span></button></a>
         */}
